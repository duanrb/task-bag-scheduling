...
public class CostSufferage extends GenericGame {

	public CostSufferage(int iClass, int iSite) {
		super(iClass, iSite);
	}
					...
					for (int k = 0; k < iaCPU[j]; k++) {
						if (tmpMinMakespan > dmMinMakespan[i][j][k]
							...
							tmpMinCost = daPrice[j] * dmPrediction[i][j];
								...
								& dmMinMakespan[i][j][k] > tmpMinMakespan
								& dDeadline >= dmMinMakespan[i][j][k]) {
	...
	public static void main(String[] args) {
		Sufferage minmin = new Sufferage(2, 2);
		minmin.test1();
	}
