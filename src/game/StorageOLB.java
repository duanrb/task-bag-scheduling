...
public class StorageOLB extends GenericStorage {

	public StorageOLB(int iClass, int iSite) {
		super(iClass, iSite);
		...
		for (int i = 0; i < iSite; i++) {
			for (int j = 0; j < iaCPU[i]; j++) {
				...
				if (tmpCompetleTime > dmMinminTime[i][j]
						&& getAvailableStorage(i, dmMinminTime[i][j]) >= daStorageInput[iMinClass]
	...
	public static void main(String[] args) {
		StorageOLB solb = new StorageOLB(2, 2);
		solb.storageTest();
