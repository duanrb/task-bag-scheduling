...
public class CostOLB extends GenericGame {

	public CostOLB(int iClass, int iSite) {
		super(iClass, iSite);
	...
	public void minOLBCost() {
		...
		while (getRestLength() > 0) {
			...
			if (iaCurrentLength[k] > 0) {
				iMinClass = k;
				findFirstAvailableCPU();
				// System.out.println("MINCLASS= "+ iMinClass+ "; MINSITE= "+
				// iMinSite+ "; MINCPU= "+ iMinCPU);
				if (!bDeadline) {
					return;
				}
				updateMin();
		...
		for (int i = 0; i < iSite; i++) {
			for (int j = 0; j < iaCPU[i]; j++) {
				if (tmpCompetleTime > dmMinminTime[i][j]
						& dDeadline >= (dmMinminTime[i][j] + dmPrediction[iMinClass][i])) {
					iMinSite = i;
					iMinCPU = j;
	...
	void updateMin() {
		...
		// System.out.println("MINMIN TIME["+iMinSite+"]["+iMinCPU+"]=" +
		// dmMinminTime[iMinSite][iMinCPU]);
